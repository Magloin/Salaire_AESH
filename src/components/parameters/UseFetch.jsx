...
function UseFetch () {
    ...
    const {loading, data, errors} = useFetch(adresse)

    let schools = data?.map((school) => {
        return <li>{school["nom_etablissement"]} - Code dep : {school["code_departement"]}</li>
    })

    function getURL(codePostal) {
        `https://data.education.gouv.fr/api/explore/v2.1/catalog/datasets/fr-en-annuaire-education/records?where=code_postal%20%3D%20%22${codePostal}%22&refine=statut_public_prive%3A%22Public%22&refine=etat%3A%22OUVERT%22&refine=ministere_tutelle%3A%22MINISTERE%20DE%20L%27EDUCATION%20NATIONALE%22`
    }
